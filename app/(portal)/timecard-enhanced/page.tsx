...
import { Plus, Edit, Trash2, Calendar, ChevronLeft, ChevronRight, BarChart3, List } from 'lucide-react';
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="flex items-center justify-between border-b border-slate-200 bg-white px-4 py-2.5">
          ...
          <div className="flex items-center gap-4">
            {role && (<span className="text-xs font-bold px-3 py-1.5 rounded-full bg-blue-100 text-blue-700">
                {role.toUpperCase()}
              </span>)}
        ...
        <div className="w-full flex-1 space-y-6 overflow-y-auto p-4">
          ...
          <div className="flex items-center justify-between">
            <h1 className="text-xl font-bold">Timecard System</h1>
            <div className="flex gap-2 bg-slate-100 p-1 rounded-lg">
              <button onClick={() => setActiveView('entries')} className={`px-4 py-2 rounded-md font-bold text-sm transition-colors flex items-center gap-2 ${activeView === 'entries'
            ? 'bg-white text-primary shadow-sm'
            : 'text-slate-600 hover:text-slate-900'}`}>
                <List size={16}/>
                Time Entries
              </button>
              <button onClick={() => setActiveView('analytics')} className={`px-4 py-2 rounded-md font-bold text-sm transition-colors flex items-center gap-2 ${activeView === 'analytics'
            ? 'bg-white text-primary shadow-sm'
            : 'text-slate-600 hover:text-slate-900'}`}>
                <BarChart3 size={16}/>
                Analytics
          ...
          {activeView === 'entries' ? (<>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                  <div className="flex items-center justify-between mb-4">
                    <button onClick={handlePreviousWeek} className="p-2 hover:bg-slate-100 rounded-lg transition-colors">
                      <ChevronLeft size={20}/>
                    </button>
                ...
                {user && role && (<WeeklySubmissionCard userId={user.id} userRole={role} weekStartDate={currentWeekStart} onUpdate={loadEntries}/>)}
              </div>

              
              {permissions?.canCreateEntry && !showEntryForm && (<button onClick={() => setShowEntryForm(true)} className="w-full py-4 border-2 border-dashed border-slate-300 rounded-xl hover:border-primary hover:bg-blue-50 transition-all flex items-center justify-center gap-2 text-slate-600 hover:text-primary font-bold">
                  <Plus size={20}/>
                  Add New Time Entry
                </button>)}

              
              {showEntryForm && user && (<div className="bg-white rounded-xl shadow-lg border border-slate-200 p-6">
                  <h3 className="text-lg font-bold mb-4">
                    {editingEntry ? 'Edit Time Entry' : 'New Time Entry'}
                  </h3>
                  <TimeEntryForm userId={user.id} initialDate={currentWeekStart} onSuccess={handleFormSuccess} onCancel={handleFormCancel} editingEntry={editingEntry}/>
                </div>)}
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-left border-collapse">
                    ...
                    <tbody className="divide-y divide-slate-200">
                      {entries.map(entry => (<tr key={entry.id} className="hover:bg-slate-50/50 transition-colors group">
                    ...
                    day: 'numeric'
                          ...
                          <td className="px-8 py-5 text-center">
                            {entry.billable ? (<span className="px-3 py-1 rounded-full text-xs font-bold bg-emerald-100 text-emerald-700">
                                Yes
                              </span>) : (<span className="px-3 py-1 rounded-full text-xs font-bold bg-slate-100 text-slate-600">
                                No
                              </span>)}
                          </td>
                          <td className="px-8 py-5 text-right">
                            <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                              {permissions?.canEditEntry && !entry.weekly_submission_id && (<button onClick={() => handleEditEntry(entry)} className="p-2 hover:bg-blue-50 text-primary rounded-lg transition-colors" title="Edit">
                                  <Edit size={16}/>
                                </button>)}
                              {permissions?.canDeleteEntry && !entry.weekly_submission_id && (<button onClick={() => handleDeleteEntry(entry.id)} className="p-2 hover:bg-red-50 text-red-600 rounded-lg transition-colors" title="Delete">
                                  <Trash2 size={16}/>
                                </button>)}
                            </div>
                          </td>
                  ...
                  {entries.length === 0 && (<div className="text-center py-12">
            ...
            </>) : (user && role && (<DashboardAnalytics userId={user.id} userRole={role}/>))}
        </div>
      </div>
    </div>);
}
