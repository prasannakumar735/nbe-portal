...
export function MaintenanceInspectionForm(props: MaintenanceFormPageProps = {}) {
  ...
  const uiVariant: 'new' | 'existing' = reportIdFromRoute ? 'existing' : 'new'
  ...
  const [activeDownloadLabel, setActiveDownloadLabel] = useState<'pdf' | 'report' | 'photos' | null>(null)
  ...
  const doorNotesOriginalRef = useRef<Record<string, string>>({})
  const doorSummaryAttemptRef = useRef<Record<string, number>>({})
  ...
  const generateDoorSummary = async (doorIndex: number) => {
    ...
    const doorKey = String(door?.local_id ?? doorIndex)
    ...
    try {
      if (!doorNotesOriginalRef.current[doorKey]) {
        doorNotesOriginalRef.current[doorKey] = doorNotes
      }
      const originalNotes = doorNotesOriginalRef.current[doorKey] || doorNotes
      const attempt = (doorSummaryAttemptRef.current[doorKey] ?? 0) + 1
      doorSummaryAttemptRef.current[doorKey] = attempt
      ...
      const response = await fetch('/api/ai/door-summary', {
        ...
        body: JSON.stringify({
          ...
          technician_notes: originalNotes,
          previous_summary: doorNotes === originalNotes ? '' : doorNotes,
          attempt,
        }),
      })
      ...
      if (!response.ok) {
        ...
        const hint = response.status === 429 && retryAfter ? ` Try again in ${retryAfter}s.` : ''
        throw new Error((data.error || 'Failed to generate AI summary') + hint)
      }
      ...
      showStatus(e instanceof Error ? e.message : 'Unable to improve notes.', 'info')
    } finally {
  ...
  return (
    <div className={`mx-auto w-full ${containerWidthClass} space-y-4 px-4 md:px-6`}>
      ...
      <form className="space-y-4 pb-24 md:pb-4" onSubmit={e => e.preventDefault()}>
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h2 className="text-sm font-bold uppercase tracking-wide text-slate-700">6. Notes (optional)</h2>
              <p className="mt-1 text-xs text-slate-500">
                Compiled from door notes. Most reports won&apos;t need this section.
              </p>
            </div>
            <button
              type="button"
            ...
            >
              {showNotesSection ? 'Hide notes' : 'Show notes'}
            </button>
          </div>
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          <h2 className="text-sm font-bold uppercase tracking-wide text-slate-700">7. Signature</h2>
          <div className="mt-3">
            <SignaturePad
            ...
            <div className="mt-3 flex flex-col gap-4">
              <div className="flex flex-wrap gap-3">
                ...
                <button
                  ...
                  onClick={async () => {
                    ...
                    try {
                      ...
                      setActiveDownloadLabel('pdf')
                      setDownloadProgress({ loaded: 0 })
                      await downloadFile(`/api/maintenance/pdf/${reportIdFromRoute}`, {
                        filenameFallback: `maintenance-report-${reportIdFromRoute.slice(0, 8)}.pdf`,
                ...
                <button
                  ...
                  disabled={activeDownloadLabel === 'report'}
                  className="h-12 rounded-xl border border-slate-300 bg-white px-5 text-base font-bold text-slate-800 hover:bg-slate-50"
                >
                  {activeDownloadLabel === 'report' ? `Downloading ${downloadProgressText}` : 'Download'}
                </button>
                <button
                  type="button"
                  onClick={async () => {
                    if (!reportIdFromRoute) return
                    try {
                      setActiveDownloadLabel('photos')
                      setDownloadProgress({ loaded: 0 })
                      await downloadFile(`/api/maintenance/photos/${reportIdFromRoute}/zip`, {
                        filenameFallback: `maintenance-photos-${reportIdFromRoute.slice(0, 8)}.zip`,
                        onProgress: p => setDownloadProgress(p),
                      })
                  ...
                  disabled={activeDownloadLabel === 'photos'}
                  className="h-12 rounded-xl border border-slate-300 bg-white px-5 text-base font-bold text-slate-800 hover:bg-slate-50"
                >
                  {activeDownloadLabel === 'photos' ? `Downloading ${downloadProgressText}` : 'Download Photos'}
                </button>
              </div>
            </div>
          ) : null}
...
// Route wrapper lives in `page.tsx`.
