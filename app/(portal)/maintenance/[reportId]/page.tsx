...
export default function MaintenanceReportEditPage() {
    ...
    const initialReportMerged = useMemo(() => {
        if (!offline.initialReportRecord)
            return null;
        return {
    ...
    const offlineMirrorStable = useMemo(() => ({ onPersist: offline.persistForm }), [offline.persistForm]);
    const accessError = useMemo(() => {
        if (offline.status !== 'ready' || !offline.reportStatus)
            return null;
    ...
    const error = authGate === 'denied'
        ? 'Sign in required.'
        : offline.status === 'error'
            ? 'Report not found or unable to load.'
            : accessError;
    if (loading) {
        return (<div className="mx-auto w-full max-w-screen-md px-4 py-8">
    ...
    return (<div className="relative">
      {offline.conflict ? (<div className="mx-auto mb-4 w-full max-w-screen-md rounded-xl border border-amber-300 bg-amber-50 px-4 py-3 text-sm text-amber-900">
          <p className="font-semibold">Sync conflict</p>
          <p className="mt-1">{offline.conflict.message}</p>
          <button type="button" className="mt-2 text-sm font-medium text-amber-950 underline" onClick={() => void offline.resolveConflictRefresh()}>
            Reload server copy and continue
          </button>
        </div>) : null}

      <MaintenanceInspectionForm reportIdFromRoute={reportId} initialReport={initialReportMerged} isAdminMode={isManagerView} serverReportStatus={offline.reportStatus} onApproved={() => router.push('/maintenance')} hydrateOnlyFromInitialReport offlineMirror={offlineMirrorStable} offlineSaveStatusLabel={formatOfflineSaveStatus(offline.saveStatus)}/>
    </div>);
}
