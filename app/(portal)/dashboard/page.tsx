...
function buildBillableStacked(entries: TimeEntryRow[]): BillableStackedDatum[] {
    const weeklyTotals = entries.reduce<Record<string, {
        billable: number;
        nonBillable: number;
    }>>((acc, entry) => {
        const date = new Date(entry.start_time);
...
export default async function DashboardPage({ searchParams, }: {
    ...
    if (!isCustomIncomplete) {
        try {
            ...
            const leaderboard = entries.reduce<Record<string, {
                hours: number;
    ...
    return (<div className="pb-2">
          <div className="sticky top-0 z-10 border-b border-gray-100 bg-gray-50/95 py-4 backdrop-blur">
            <DashboardHeader title="Dashboard" subtitle="Time insights & performance overview" actions={<FilterBar projects={projectOptions} period={period} project={projectId} startDate={qp.start ?? ''} endDate={qp.end ?? ''}/>}/>
          </div>

          <div className="space-y-6 pt-4">
            {errorMessage ? (<DashboardErrorState message={errorMessage}/>) : isCustomIncomplete ? (<DashboardErrorState message="Select a custom date range to view analytics."/>) : (<Suspense fallback={<LoadingSkeleton rows={3}/>}>
                {role === 'admin' || role === 'manager' ? (<AdminDashboard data={adminData} charts={charts} entriesCount={entries.length} employeeLeaderboard={employeeLeaderboard} recentEntries={recentEntries}/>) : (<EmployeeDashboard data={employeeData} charts={charts} entriesCount={entries.length}/>)}
              </Suspense>)}
          </div>
        </div>);
}
