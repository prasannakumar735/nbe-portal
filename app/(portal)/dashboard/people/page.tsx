...
function tabFromSearch(raw: string | undefined, canManageClients: boolean): PeopleTabId {
    if (raw === 'contacts')
        return 'contacts';
    if (raw === 'clients' && canManageClients)
        return 'clients';
    return 'users';
}
...
export default async function DashboardPeoplePage({ searchParams, }: {
    ...
    return (<div>
      <Suspense fallback={<PeopleTabsFallback />}>
        <PeopleTabs initialTab={initialTab} initialUsers={users} usersError={usersError} currentUserId={sessionUser?.id ?? ''} initialClients={initialClients} clientsError={clientsError} canManageClients={canManageClients}/>
      </Suspense>
    </div>);
}
