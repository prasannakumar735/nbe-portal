    ...
    return (<div className="w-full">
      <header className="mb-4 flex flex-col gap-4 border-b border-slate-200 pb-4 print:hidden">
        <div className="flex flex-col items-start justify-between gap-3 sm:flex-row sm:items-center">
          ...
          <button type="button" onClick={handlePrint} className="inline-flex h-11 items-center justify-center rounded-xl bg-slate-900 px-4 text-sm font-semibold text-white shadow-sm hover:bg-slate-800">
            Print QR Stickers
          </button>
        </div>
        ...
        <div className="mt-2 flex flex-wrap gap-3 text-sm">
          ...
          <label className="flex items-center gap-2">
            ...
            <select value={selectedLocationId} onChange={event => setSelectedLocationId(event.target.value)} className="h-9 rounded-lg border border-slate-300 bg-white px-2 text-sm" disabled={!selectedClientId}>
      ...
      {isLoading ? (<p className="text-sm text-slate-600">Loading doors…</p>) : error ? (<p className="text-sm font-semibold text-red-600">{error}</p>) : doors.length === 0 ? (<p className="text-sm text-slate-600">No doors found.</p>) : (<main className="print:mt-0">
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3 print:grid print:grid-cols-3 print:gap-6">
            {doors.map(door => {
                const suburb = (door.client_locations && 'suburb' in door.client_locations
                    ? door.client_locations.suburb
                    : null) || 'N/A';
                return (<section key={door.id} className="flex h-64 w-64 flex-col items-center justify-between rounded-xl border border-slate-300 bg-white p-3 text-center shadow-sm print:h-64 print:w-64">
                  <div>
                  ...
                  <div className="flex flex-1 items-center justify-center">
                    <img src={`/api/qr/door/${encodeURIComponent(door.id)}`} alt={`QR code for door ${door.door_label || door.id}`} className="h-28 w-28"/>
                  </div>
