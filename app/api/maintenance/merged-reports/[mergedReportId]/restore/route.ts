...
import { restoreMergedReport } from '@/lib/merged-reports/deleteMergedReport'
...
export async function POST(
  ...
  try {
    ...
    const result = await restoreMergedReport(supabase, mergedReportId.trim())

    if (!result.ok) {
      if (result.code === 'not_deleted') {
        return NextResponse.json({ error: 'Report is not deleted' }, { status: 409 })
      }
    ...
    const message = err instanceof Error ? err.message : 'Failed to restore merged report'
    console.error('[POST merged-reports/restore]', err)
