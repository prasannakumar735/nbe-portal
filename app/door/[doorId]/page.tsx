...
export default async function DoorInfoPage({ params }: DoorPageProps) {
    ...
    if (clientLocationId) {
        ...
        if (location) {
            locationLabel = String(location.location_name ?? location.name ?? location.suburb ?? location.site_name ?? location.address ?? '').trim() || 'Not available';
    ...
    if (latestInspection?.report_id) {
        const { data: reportData } = await serviceSupabase
            ...
            .maybeSingle();
        const technicianName = String((reportData as {
            technician_name?: string | null;
        } | null)?.technician_name ?? '').trim();
        if (technicianName) {
      ...
      <div className="mx-auto flex w-full max-w-xl flex-col gap-4">
        <header className="text-center">
          <div className="mb-8 flex flex-col items-center">
            <Image src="/nbe-logo.png" alt="NBE Australia" width={220} height={72} className="object-contain" priority/>
        ...
        <main className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <div className="space-y-5">
            ...
            {!user ? (<section className="rounded-xl border border-slate-200 bg-slate-50 p-3">
              ...
              </section>) : (<section className="rounded-xl border border-slate-200 bg-slate-50 p-3">
                <h3 className="text-sm font-bold text-slate-800">Previous Inspection</h3>
                {latestInspection ? (<div className="mt-2 space-y-1 text-sm text-slate-700">
                  ...
                  </div>) : (<p className="mt-1 text-sm text-slate-600">Previous Inspection Notes: NA</p>)}
              </section>)}

            {canStartInspection && (<section className="pt-2">
                <Link href={`/maintenance/new?door_id=${encodeURIComponent(doorId)}`} className="inline-flex h-11 items-center justify-center rounded-xl bg-slate-900 px-6 py-3 text-sm font-semibold text-white hover:bg-slate-800">
