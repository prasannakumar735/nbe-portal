...
export async function runMaintenanceSubmit(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    if (!saveResponse.ok) {
      throw new Error((saved as { error?: string }).error || 'Unable to submit report')
    }

    const reportId = (saved as { report_id: string }).report_id as string
    ...
    return jsonError500(error, 'maintenance-submit')
  }
}
